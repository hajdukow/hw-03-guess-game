...
document.querySelector('.btn-info').addEventListener('click', function () {
    ...
    minValue = minValue > 999 ? 999 : minValue && minValue < -999 ? -999 : minValue;
    maxValue = maxValue > 999 ? 999 : maxValue && maxValue < -999 ? -999 : maxValue;
    ...
    document.querySelector('#btnOver').addEventListener('click', function () {
        if (gameRun) {
            if (minValue === maxValue) {
                ...
                const phraseRandom = Math.round(Math.random());
                const answerPhrase = (phraseRandom === 1) ?
                    `Вы указали числа неверно!\n\u{1F914}` :
                    `Упс.. Начните игру заново\n\u{1F92F}`;

                answerField.innerText = answerPhrase;
                alertText.innerText = `Минимальное значение ${minValue} больше максимального ${maxValue}`;
                gameRun = false;
            } else if (minValue !== maxValue) {
